...
if [ -z "$ARIA2C_RPC_SERVER_PID" ]; then
    ...
    nohup ./aria2c --enable-rpc --rpc-listen-all --ca-certificate=./ca-certificates.crt --dir=/app/public/downloads --input-file=aria_session.txt --save-session=aria_session.txt --save-session-interval=3 --max-connection-per-server=16 > /dev/null 2>&1 < /dev/null &
